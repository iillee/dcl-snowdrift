...
- **Local multi-player**: the desktop Explorer runs **one instance at a time** — pressing Preview again only re-focuses the window that is already open, it does not add a player. Either (a) tick **Multi-Instance Preview** in Creator Hub's Preview Options dropdown (= `sdk-commands start --multi-instance`; needs `@dcl/sdk` >= 7.20.4) and press Preview again — multi-instance forces a separate login per window, so it turns Skip Auth Screen off and each instance must sign in with a different account; or (b) keep the desktop client open and add a browser client as player 2 — Preview Options → *Preview with: Bevy (Web)* (= `sdk-commands start --web`, alias `--bevy-web`). `-n` also forces a new desktop window on its own.
